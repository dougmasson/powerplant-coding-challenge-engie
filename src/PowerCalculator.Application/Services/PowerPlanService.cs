...
namespace PowerCalculator.Application.Services
{
    public class PowerPlanService : IPowerPlanService
    {
        ...
        /// <summary>
        /// Create a powerPlant base on informations of <see cref="PowerPlantInfo"/> and <see cref="FuelsInfo"/>
        /// </summary>
        ...
        internal void SortByMeritOrder(ref List<PowerPlant> powerPlants)
        {
            ...
            var orderByOthersPowerPlant = powerPlants.Where(x => x.PowerCapacityToGenerate == 0)
                                                     .OrderBy(x => x.PowerCost)
                                                     .ThenBy(i => i.PowerMinimum);

            powerPlants = orderByPowerPlantAbleToGenerate.Concat(orderByOthersPowerPlant).ToList();
        }

        /// <summary>
        /// Calculate how much energy needs to produce for each power plant .
        /// </summary>
        /// <param name="powerPlants"></param>
        /// <param name="load"></param>
        internal async Task<List<ProductionPlan>> ComputeProductionPlanAsync(List<PowerPlant> powerPlants, double load)
        {
            _logger.LogInformation("Calculate production plan");

            double remainingLoad = load;

            for (int i = 0; i < powerPlants.Count; i++)
            {
                if (remainingLoad < powerPlants[i].PowerMinimum)
                {
                ...
                remainingLoad = await CalculatePowerGeneratedForPlanAsync(powerPlants[i], remainingLoad);
        ...
        /// <summary>
        /// Calculate power generated for plan.
        /// </summary>
        /// <param name="powerPlant"></param>
        /// <param name="remainingLoad"></param>
        /// <returns>Value of <c>remainingLoad</c> after calcule.</returns>
        internal Task<double> CalculatePowerGeneratedForPlanAsync(PowerPlant powerPlant, double remainingLoad)
        {
        ...
        internal Task<double> RedistributePowerPlanGeneratedAsync(List<PowerPlant> powerPlants, double remainingLoad, int indexCurrentPowerPlant)
        {
        ...
        internal void RollbackRedistributePowerPlanGenerated(List<PowerPlant> powerPlants, int i)
        {
